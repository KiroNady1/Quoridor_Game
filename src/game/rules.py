...
def is_wall_placement_valid(
    board, wall: Wall, players: list[Player]
) -> tuple[bool, str]:
    """
    Returns (ok, reason).
    ...
    try:
        for p in players:
            has_path = bfs_path_exists(board, p)

            print(
                f"Player at {p.r},{p.c} goal row " f"{p.goal_row} has path: {has_path}"
            )

            if not has_path:
                return False, "Would trap a player"

        return True, ""

    finally:
        # FIX: deterministic cleanup instead of pop()
        if wall in board.walls:
